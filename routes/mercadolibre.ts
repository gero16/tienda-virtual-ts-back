...
// =====================
// 🔧 Sincronizar TODOS los productos para detectar y actualizar descuentos ML
// Busca todos los productos y verifica si tienen descuento en ML
// =====================
router.post("/sync/detectar-descuentos-ml", async (req: Request, res: Response) => {
  try {
    const token = await getCurrentToken();
    if (!token) throw new Error("No autenticado");

    const limit = parseInt(req.query.limit as string) || 100; // Por defecto, 100 productos

    console.log(`🔄 Iniciando detección de descuentos ML para hasta ${limit} productos...`);

    // Buscar productos activos (no archivados, no ocultos)
    const productos = await Producto.find({
      archivado: { $ne: true },
      hidden_by_price_issue: { $ne: true },
      status: { $in: ['active', 'paused'] }
    })
    .select("ml_id title price descuento_ml descuento")
    .limit(limit);

    console.log(`📦 Encontrados ${productos.length} productos para verificar`);

    const resultados = [];
    let actualizados = 0;
    let nuevosDescuentos = 0;
    let sinCambios = 0;
    let errores = 0;
    ...
    setTimeout(async () => {
      for (const producto of productos) {
        try {
          ...
          // Verificar si tiene descuento ML
          const tieneDescuentoML = item.original_price && item.original_price !== item.price;
          ...
          if (tieneDescuentoML) {
            ...
            const precioOriginalBD = producto.descuento_ml?.original_price;
            const necesitaActualizacion = 
              !precioOriginalBD || // No tiene descuento_ml guardado
              (precioOriginalBD && Math.abs(precioActualBD - precioOriginalBD) < 0.01) || // Precio BD = precio original
              (producto.descuento?.activo === false && Math.abs(precioActualBD - precioRebajadoML) > 0.01); // Precio BD ≠ precio rebajado ML

            if (necesitaActualizacion) {
              const esNuevoDescuento = !precioOriginalBD;
              if (esNuevoDescuento) nuevosDescuentos++;

              console.log(`  ${esNuevoDescuento ? '🆕' : '⚠️'} ${esNuevoDescuento ? 'Nuevo descuento ML detectado' : 'Precio incorrecto detectado'}: ${producto.title}`);
              console.log(`     Precio en BD: $${precioActualBD}`);
              console.log(`     Precio original ML: $${precioOriginalML}`);
              console.log(`     Precio rebajado ML: $${precioRebajadoML}`);
              ...
              resultados.push({
                ...
                title: producto.title,
                actualizado: true,
                es_nuevo_descuento: esNuevoDescuento,
                precio_anterior: precioActualBD,
                precio_nuevo: precioRebajadoML,
                precio_original: precioOriginalML
              });
            ...
            // No tiene descuento ML, pero si tenía descuento_ml guardado, limpiarlo
            if (producto.descuento_ml?.original_price) {
              await Producto.updateOne(
                { ml_id: producto.ml_id },
                { $unset: { descuento_ml: "" } }
              );
      ...
      console.log("\n✅ Detección de descuentos ML completada:");
      console.log(`   ✅ Productos actualizados: ${actualizados}`);
      console.log(`   🆕 Nuevos descuentos detectados: ${nuevosDescuentos}`);
      console.log(`   ✓ Sin cambios: ${sinCambios}`);
      console.log(`   ❌ Errores: ${errores}`);
    }, 100);

    res.json({
      mensaje: `Detección de descuentos ML iniciada en background (hasta ${limit} productos)`,
      total_productos: productos.length,
      status: "running",
      nota: "Revisa los logs del servidor para ver el progreso. Los productos se están verificando en segundo plano."
    });

  } catch (error: any) {
    console.error("❌ Error iniciando detección de descuentos ML:", error);
    res.status(500).json({
      error: "Error al iniciar detección de descuentos ML",
      details: error.message
    });
  }
});
